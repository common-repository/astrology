...
use Prokerala\Astrology\Vendor\Predis\Connection\Aggregate\PredisCluster;
...
use Prokerala\Astrology\Vendor\Symfony\Component\Cache\Exception\LogicException;
use Prokerala\Astrology\Vendor\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use Prokerala\Astrology\Vendor\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use Prokerala\Astrology\Vendor\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    {
        if ($redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
        ...
        if (\defined('Prokerala\Astrology\Vendor\Redis::OPT_COMPRESSION') && ($redis instanceof \Redis || $redis instanceof \RedisArray || $redis instanceof \RedisCluster)) {
...
EOLUA;
        $results = $this->pipeline(function () use ($ids, $lua) {
            foreach ($ids as $id) {
                yield 'eval' => $this->redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1];
            }
        });
        foreach ($results as $id => $result) {
...
EOLUA;
        $results = $this->pipeline(function () use ($tagIds, $lua) {
            ...
            foreach ($tagIds as $id) {
                yield 'eval' => $this->redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1];
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()
