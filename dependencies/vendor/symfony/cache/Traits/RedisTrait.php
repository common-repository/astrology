...
use Prokerala\Astrology\Vendor\Predis\Connection\Aggregate\ClusterInterface;
use Prokerala\Astrology\Vendor\Predis\Connection\Aggregate\RedisCluster;
use Prokerala\Astrology\Vendor\Predis\Connection\Aggregate\ReplicationInterface;
use Prokerala\Astrology\Vendor\Predis\Response\ErrorInterface;
use Prokerala\Astrology\Vendor\Predis\Response\Status;
use Prokerala\Astrology\Vendor\Symfony\Component\Cache\Exception\CacheException;
...
trait RedisTrait
{
    ...
    private function init($redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        ...
        if (!$redis instanceof \Redis && !$redis instanceof \RedisArray && !$redis instanceof \RedisCluster && !$redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface && !$redis instanceof RedisProxy && !$redis instanceof RedisClusterProxy) {
            throw new InvalidArgumentException(sprintf('"%s()" expects parameter 1 to be Redis, RedisArray, RedisCluster or Predis\ClientInterface, "%s" given.', __METHOD__, \is_object($redis) ? \get_class($redis) : \gettype($redis)));
        }
        if ($redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface && $redis->getOptions()->exceptions) {
    ...
    public static function createConnection($dsn, array $options = [])
    {
        ...
        if (!\extension_loaded('redis') && !class_exists(\Prokerala\Astrology\Vendor\Predis\Client::class)) {
        ...
        if (isset($params['redis_sentinel']) && !class_exists(\Prokerala\Astrology\Vendor\Predis\Client::class)) {
        ...
        if (null === $params['class'] && !isset($params['redis_sentinel']) && \extension_loaded('redis')) {
            ...
            $class = $params['class'] ?? \Prokerala\Astrology\Vendor\Predis\Client::class;
            if (isset($params['redis_sentinel']) && !is_a($class, \Prokerala\Astrology\Vendor\Predis\Client::class, \true)) {
        ...
        if (is_a($class, \Redis::class, \true)) {
            ...
            $initializer = static function ($redis) use ($connect, $params, $dsn, $auth, $hosts, $tls) {
                ...
                try {
                    @$redis->{$connect}($host, $port, $params['timeout'], (string) $params['persistent_id'], $params['retry_interval'], $params['read_timeout'], ...\defined('Prokerala\Astrology\Vendor\Redis::SCAN_PREFIX') ? [['stream' => $params['ssl'] ?? null]] : []);
                    ...
                    if (0 < $params['tcp_keepalive'] && \defined('Prokerala\Astrology\Vendor\Redis::OPT_TCP_KEEPALIVE')) {
            ...
            $initializer = static function () use ($class, $params, $dsn, $hosts) {
                ...
                try {
                    $redis = new $class(null, $hosts, $params['timeout'], $params['read_timeout'], (bool) $params['persistent'], $params['auth'] ?? '', ...\defined('Prokerala\Astrology\Vendor\Redis::SCAN_PREFIX') ? [$params['ssl'] ?? null] : []);
                } catch (\RedisClusterException $e) {
                    throw new InvalidArgumentException(sprintf('Redis connection "%s" failed: ', $dsn) . $e->getMessage());
                }
                if (0 < $params['tcp_keepalive'] && \defined('Prokerala\Astrology\Vendor\Redis::OPT_TCP_KEEPALIVE')) {
        ...
        } elseif (is_a($class, \Prokerala\Astrology\Vendor\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface && $this->redis->getConnection() instanceof ClusterInterface) {
    ...
    {
        if ($this->redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface) {
        ...
        $hosts = $this->getHosts();
        $host = reset($hosts);
        if ($host instanceof \Prokerala\Astrology\Vendor\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
        ...
        foreach ($hosts as $host) {
            ...
            if (!$host instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface) {
                $prefix = \defined('Prokerala\Astrology\Vendor\Redis::SCAN_PREFIX') && \Redis::SCAN_PREFIX & $host->getOption(\Redis::OPT_SCAN) ? '' : $host->getOption(\Redis::OPT_PREFIX);
            ...
            if (!version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface ? $host->scan($cursor, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    private function pipeline(\Closure $generator, $redis = null): \Generator
    {
        ...
        if ($redis instanceof RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface && $redis->getConnection() instanceof RedisCluster) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
        ...
        if ($this->redis instanceof \Prokerala\Astrology\Vendor\Predis\ClientInterface) {
            ...
            if ($connection instanceof ClusterInterface && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \Prokerala\Astrology\Vendor\Predis\Client($c);
                }
            }
