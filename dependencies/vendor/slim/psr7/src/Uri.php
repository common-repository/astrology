...
class Uri implements UriInterface
{
    ...
    protected function filterUserInfo(?string $info = null): string
    {
        ...
        $match = preg_replace_callback('/(?:[^%a-zA-Z0-9_\-\.~\pL!\$&\'\(\)\*\+,;=]+|%(?![A-Fa-f0-9]{2}))/u', function ($match) {
    ...
    {
        $match = preg_replace_callback('/(?:[^a-zA-Z0-9_\-\.~:@&=\+\$,\/;%]+|%(?![A-Fa-f0-9]{2}))/', function (array $match) {
            return rawurlencode($match[0]);
        }, $path);
        return is_string($match) ? $match : '';
    }
