...
use Prokerala\Astrology\Vendor\Buzz\Exception\CallbackException;
use Prokerala\Astrology\Vendor\Buzz\Exception\ClientException;
use Prokerala\Astrology\Vendor\Buzz\Exception\NetworkException;
use Prokerala\Astrology\Vendor\Buzz\Exception\RequestException;
use Prokerala\Astrology\Vendor\Buzz\Message\HeaderConverter;
use Prokerala\Astrology\Vendor\Buzz\Message\ResponseBuilder;
...
abstract class AbstractCurl extends AbstractClient
{
    ...
    {
        $options = [\CURLOPT_CUSTOMREQUEST => $request->getMethod(), \CURLOPT_URL => $request->getUri()->__toString(), \CURLOPT_HTTPHEADER => HeaderConverter::toBuzzHeaders($request->getHeaders())];
